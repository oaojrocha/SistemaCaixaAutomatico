...
public class ControladorCaixa {
    ...
    public float consultarSaldo(int num, int senha) {
        final ContaCor cta = dbContas.buscarConta(num); // obtem referencia para o objeto que representa a conta 'num'
        float valTaxa = cta.getTipoConta().calcularTaxa();
        if (cta == null)   // se numero de conta invalido ...
            return -1; // ... retorna -1
        else {             // caso contrario ...
            if (valTaxa > 0) {
                cta.cobrarTaxa("Taxa de consulta de saldo", valTaxa, senha);
            }
    ...
    public boolean efetuarSaque(int num, int senha, float val) {
        ...
        final ContaCor cta = dbContas.buscarConta(num);  // obtem a referencia para o objeto que representa a conta 'num'
        float valTaxa = cta.getTipoConta().calcularTaxa();

        float saldoCaixa = this.caixa.obterSaldoCaixa();

        if (saldoCaixa < val + valTaxa) {
        ...
        if (cta.debitarValor("Saque Automatico", val, senha) == false) // se saque recusado ...
            ...
            if (valTaxa > 0) {
                cta.cobrarTaxa("Taxa de saque", valTaxa, senha);
            }
    ...
    public boolean efetuarTransferencia(int num, int senha, float val, int numDest) {

        ContaCor cta = dbContas.buscarConta(num);  // obtem a referencia para o objeto que representa a conta 'num'
        float saldoConta = cta.obterSaldo(senha);
        float valTaxa = cta.getTipoConta().calcularTaxa();


        if (saldoConta < val + valTaxa) {
            System.out.println("Saldo insuficiente.");
            return false;
        }

        ContaCor ctaDest = dbContas.buscarConta(numDest);  // obtem a referencia para o objeto que representa a conta 'num'

        if (cta == null)  // se número de conta inválido ...
            return false;  // ... retorna false

        if (ctaDest == null)  // se número de conta inválido ...
            return false;  // ... retorna false

        if (cta.debitarValor("Trânsferencia entre contas", val, senha) == false) // se saque recusado ...
            return false;  // retorna false
        else {
            if (valTaxa > 0) {
                cta.cobrarTaxa("Taxa de trânsferencia", valTaxa, senha);
            }
            ctaDest.creditarValor("Trânsferencia entre contas", val);
            return true;
        }
    }
