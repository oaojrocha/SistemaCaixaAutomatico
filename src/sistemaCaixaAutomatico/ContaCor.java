...
public class ContaCor {
    ...
    private TipoContaStrategy tipoConta;          // Básica ou Premium
    ...
    public ContaCor(String titular, float saldoAtual, int numConta, int senha, TipoContaStrategy tipoConta) {
        this.estado = ContaCor.ATIVA;        // A conta se torna ativa, ppodendo receber lançamentos.
        this.tipoConta = tipoConta;        // Tipo de conta
    ...
    public boolean cobrarTaxa(String hist, float val, int senhaCliente) {
    ...
    public boolean creditarValor(String hist, float val) {
        ...
        if (saldoAtual > 0 && estado == ContaCor.ENCERRADA) {            // se zerou o saldo ...
            estado = ContaCor.ATIVA;        // ... reativa a conta
            System.out.println("Conta de " + this.titular + ", número " + this.numConta + " foi reativada.");
        }
        return true;
    }

    public TipoContaStrategy getTipoConta() {
        return tipoConta;
    }
}
