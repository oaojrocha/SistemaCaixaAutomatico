...
public class TrmCxAut {
    ...
    private int getOp() {
        ...
        do {
            if (modoAtual == Caixa.MODO_CLIENTE) {  // modo cliente
                op = getInt
                        ("opcao: 1 = consulta saldo, 2 = saque, 4 = trânsferencia entre contas, 8 = modo supervisor, 9 = sai");
    ...
    private void transferenciaEntreContas() {
        ...
        boolean transferenciaEfetuada = controladorCaixa.efetuarTransferencia(getInt("número da conta"), getInt("senha"), getInt("valor"), getInt("número da conta distino"));
        if (transferenciaEfetuada)
            System.out.println("Trânsferencia Efetuada");
        else
